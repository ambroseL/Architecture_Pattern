...
void BallToBrick::doStrategy()
{
	...
	if (abs(position.x + 50.0f / pixToMeter - pos->points->x) <= 1 && abs(position.y + 20.0f / pixToMeter - pos->points->y) <= 1)//右上角
	{
		if (vec.y < 0)
		{
			vec.x *= 1.2f;
	...
	if (abs(position.x - 50.0f / pixToMeter - pos->points->x) <= 1 && abs(position.y - 20.0f / pixToMeter - pos->points->y) <= 1)//左下角
	{
		if (vec.y > 0)
		{
			vec.x *= 1.2f;
	...
	if (abs(position.x - 50.0f / pixToMeter - pos->points->x) <= 1 && abs(position.y + 20.0f / pixToMeter - pos->points->y) <= 1)//左上角
	{
		if (vec.y < 0)
		{
			vec.x *= 1.2f;
...
void PaddleToBall::doStrategy()
{
	...
	d = objManager->getPaddleInitialWidth() * 2;
